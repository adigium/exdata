...
export interface DepthLightSpecification<TWebsocketMessages extends Websocket.Exchange.Topic> {
  /** Indicates:
  ...
  mapDepthData(data: TWebsocketMessages[Websocket.Topic.DEPTH_LIGHT]): Topic.Unified.Depth[];
  /** Exctracts subjects (symbols) from the incoming data. Often it's just one subject per data message. */
  getDepthSubjects(data: TWebsocketMessages[Websocket.Topic.DEPTH_LIGHT]): string[] | undefined;
}
