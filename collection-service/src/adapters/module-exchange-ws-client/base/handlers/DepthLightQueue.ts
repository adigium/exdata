...
export class DepthLightQueue<TWebsocketMessages extends Websocket.Exchange.Topic>
  implements WebsocketScoped, TopicRelated<TWebsocketMessages>
{
  ...
  private queue: AggregatedEventQueue<TWebsocketMessages[Websocket.Topic.DEPTH_LIGHT]>;
  ...
  constructor(
    ...
    this.queue = new AggregatedEventQueue<TWebsocketMessages[Websocket.Topic.DEPTH_LIGHT]>(
  ...
  public enqueue(subject: string, data: TWebsocketMessages[Websocket.Topic.DEPTH_LIGHT]) {
    this.queue.enqueue(subject, data);
  }

  private async handleQueueMessage(subject: string, data: TWebsocketMessages[Websocket.Topic.DEPTH_LIGHT]) {
  ...
  private async _synchronize() {
    ...
    try {
      ...
      const activeStreams: [string, number][] = clientWs
        ...
        .filter(
          (stream) =>
            stream.innerSubject && this.symbolsInnerUnifiedMap.has(stream.innerSubject.toLowerCase()),
        )
  ...
  private async process(props: {
    subject: string;
    data: TWebsocketMessages[Websocket.Topic.DEPTH_LIGHT];
  }): Promise<boolean> {
      ...
      return mappedData.reduce((prevResult, dataRecord) => {
        ...
        if (!isFirstProcessedEvent && !isAllowedToProcess && !isLooselyProcessed) {
          ...
          clientWs
            ...
            .then(() => clientWs.subscribe({ topic: this.topic, symbols: [subject] }));
          return prevResult && false;
        }
