...
export type CurrenciesResponse<R = any, N = any> = {
  [currency: string]: Currency<R, N>;
};

export type Currency<R = any, N = any> = {
  ...
  //   };
  networks: {
    [network: string]: Network<N>;
  }; // network structures indexed by unified network identifiers (ERC20, TRC20, BSC, etc)
  info: R; // the original unparsed currency info from the exchange
};

export type Network<R = any> = {
  id: string; // string literal for referencing within an exchange
...
export type BalancesResponse<R = any> = {
  info: R; // the original untouched non-parsed reply with details
  timestamp: number; // Unix Timestamp in milliseconds (seconds * 1000)
  datetime: string; // ISO8601 datetime string with milliseconds
  ...
  data: {
    [accountType: string]: {
      [code: string]: {
        ...
        code: string;
        free: string;
        used: string; // money on hold, locked, frozen, or pending, by currency
        total: string; // total (free + used), by currency
      };
    };
  };
...
export type DepositWithdrawalFee<R = any> = {
...
export type DepositWithdrawalFeesResponse<R = any> = {
  [currency: string]: DepositWithdrawalFee<R>;
};
